...
|---|---|---|---|
| `POST` | `/records` | 건강 기록 추가 | 구현 예정 |
| `GET` | `/records` | 전체 건강 기록 조회 | 구현 예정 |
| `GET` | `/records/{record_id}` | 특정 건강 기록 조회 | 구현 예정 |
| `PUT` | `/records/{record_id}` | 특정 건강 기록 수정 | 구현 예정 |
| `DELETE` | `/records/{record_id}` | 특정 건강 기록 삭제 | 구현 예정 |
...
- 하나의 기록에는 고유한 ID를 부여합니다.
- 삭제된 ID는 다시 사용하지 않습니다.
- 기록을 수정하면 BMI와 건강 분류 결과를 다시 계산합니다.
