...
- 시작일과 종료일을 모두 포함한 날짜 범위 검색
- 전체 건강 기록 개수 제공
- 평균 체중 및 평균 BMI 제공
- 평균 수축기·이완기 혈압 제공
- 평균 공복 혈당 제공
- 평균 걸음 수 및 평균 수면 시간 제공
- 저장된 기록이 없으면 통계값을 `null`로 반환
...
- 통계는 현재 저장된 전체 건강 기록을 기준으로 계산합니다.
- 평균값은 소수점 둘째 자리까지 반올림합니다.
- 저장된 기록이 없으면 기록 수는 `0`, 평균 통계값은 `null`로 반환합니다.
...
다음 기능을 Docker 환경에서 최종 확인했습니다.

- 기본 API 및 Swagger 문서 접속
- 건강 기록 추가·조회·수정·삭제
- BMI, 혈압, 공복 혈당 자동 분석
- 건강 상태에 따른 경고 메시지 생성
- 날짜 범위 검색
- 전체 건강 기록 평균 통계
- 존재하지 않는 기록 요청 시 `404` 응답
- 잘못된 날짜 검색 범위 요청 시 `400` 응답
- 입력값 검증 실패 시 `422` 응답
- 서버 및 컨테이너 재시작 후 JSON 데이터 유지

모든 필수 기능이 정상적으로 작동하는 것을 확인했습니다.
